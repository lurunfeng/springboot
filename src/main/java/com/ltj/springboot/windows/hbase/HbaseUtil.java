package com.ltj.springboot.windows.hbase;
...
public class HbaseUtil {
	...
	static {
		...
		/*
		 * config = HBaseConfiguration.create(); config.set("hbase.zookeeper.quorum",
		 * "127.0.0.1"); config.set("hbase.zookeeper.property.clientPort", "2181");
	...
	public ResultScanner queryTableByCondition(String tabName, Filter filter) {
		...
		try {
			...
			// Bytes.toBytes("name"), CompareOp.EQUAL, Bytes.toBytes("bookName6"));
			// 创建一个数据表扫描器
			Scan scan = new Scan();
			// 将查询过滤器加入到数据表扫描器对象
